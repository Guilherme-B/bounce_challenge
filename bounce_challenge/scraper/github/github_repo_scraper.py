...
from bounce_challenge.scraper.base.data_accumulator import DataAccumulator, DataOutputType
...
class GithubRepoScraper(BaseScraper):
    ...
    def start(self, **kwargs) -> bool:
        ...
        session: Session = self._authenticate() if self._auth_method is not None else Session()
        ...
        user_profile_link: str = self._build_user_repository_url(user_name=github_user)
        ...
        self._exhaust_requests(target_url=user_profile_link, session=session, output_callback=data_accumulator.add_json_data)
        # dump the provided information as a CSV
        data_accumulator.dump(output_path=output_path, output_type=DataOutputType.CSV, data_filters=data_filters)

        return True

    def _build_user_repository_url(self: Type[BaseScraper], user_name: str) -> str:
        user_repositories_url: str = GithubRepoScraper._USER_REPOSITORIES_URL.format(username=user_name)
        user_profile_link: str = urljoin(GithubRepoScraper._API_URL, user_repositories_url)
    ...
    def _authenticate(self: Type[BaseScraper]) -> Session:
        ...
        if isinstance(auth_method, AuthMethodToken):
            ...
            raise NotImplementedError(f"Authentication not implemented for method {auth_method}")
